...
class _NewChatPageState extends State<NewChatPage> {
  ...
  _checkChat(String idd, String name) async {
    final result = await SupabaseHandler()
        .client
        .from('chat')
        .select()
        .or('sender.eq.${idd},rec.eq.${idd}')
        .execute();

    final datalist = result.data as List;

    if (datalist.length > 0) {
      Navigator.of(context).push(ChatPage.route(datalist[0]["id"], idd, name));
    } else {

      final resulttt = await SupabaseHandler()
          .client
          .from("chat")
          .insert({'rec': idd, 'sender': 1}).execute();

      Navigator.of(context).push(ChatPage.route(
          resulttt.data[0]["id"], idd.toString(), name.toString()));
    }
  }
